...
```bash
docker build -f src/main/docker/Dockerfile -t artemiskamelet:1.0-SNAPSHOT .
```

Once the application is published, you can run it directly from the container:

```bash
docker run -it artemiskamelet:1.0-SNAPSHOT
```
...
```bash
./artemiskamelet-1.0-SNAPSHOT-runner
```
...
```bash
docker build -f src/main/docker/Dockerfile.native -t native-artemiskamelet:1.0-SNAPSHOT .
```

Once the application is published, you can run it directly from the container:

```bash
docker run -it native-artemiskamelet:1.0-SNAPSHOT
```
